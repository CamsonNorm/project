...
$sql = "SELECT customer_name, bookingID   , user_id, customer_phone, customer_email, goods_type, description, weight,
        pickup_location, dropoff_location, booking_date, vehicleID, vehicle_type, vehicle_plate FROM transactioninfor WHERE transaction_status = 'nortSorted'";
...
foreach($TransactionInfor as $Transaction){
    echo implode(',', [$Transaction['bookingID'], $Transaction['customer_name'], $Transaction['customer_email'], $Transaction['customer_phone'], 
    $Transaction['goods_type'], $Transaction['weight'], $Transaction['description'], $Transaction['pickup_location'], $Transaction['dropoff_location'], $Transaction['vehicleID'],  $Transaction['vehicle_type'],  $Transaction['vehicle_plate']]). "\n";
